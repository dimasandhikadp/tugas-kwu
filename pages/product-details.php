        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-6 md:p-8 flex flex-col">
            ...
            <form action="checkout.php" method="GET" id="form-checkout">
                ...
                <div class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center mb-4">
                    ...
                    <button type="button" class="flex-1 h-11 bg-blue-600 text-white font-semibold rounded-lg text-sm flex items-center justify-center gap-2 hover:bg-blue-700 transition shadow-sm shadow-blue-600/10 cursor-pointer">
        ...
        ?>
                <a href="product-details.php?produk=<?= $row_serupa['slug']; ?>" class="w-full bg-white border border-slate-200 rounded-2xl p-3.5 hover:shadow-sm hover:-translate-y-[2px] transition-all duration-200 relative flex flex-col group">
                  <div class="absolute top-3 left-3 z-10">
                    <span class="<?= $s_badge_class; ?> text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider border"><?= $s_badge_text; ?></span>
                  </div>
                  <div class="w-full aspect-square flex items-center justify-center overflow-hidden rounded-xl mb-2.5 bg-slate-50">
                    <?php if (!empty($row_serupa['gambar_utama']) && file_exists("../assets/img/product-image/" . $row_serupa['gambar_utama'])): ?>
                        <img src="../assets/img/product-image/<?= $row_serupa['gambar_utama']; ?>" class="w-full h-full object-cover">
                    <?php else: ?>
                        <i data-lucide="fish" class="w-14 h-14 text-blue-500/70"></i>
                    <?php endif; ?>
                  </div>
                  <div class="space-y-0.5 mb-2.5 flex-1">
                    <h4 class="font-bold text-slate-800 text-sm truncate"><?= htmlspecialchars($row_serupa['nama_produk']); ?></h4>
                    <p class="text-[11px] text-slate-400"><?= number_format($row_serupa['berat'], 0); ?> <?= htmlspecialchars($row_serupa['satuan'] ?? 'kg'); ?></p>
                  </div>
                  <div class="flex items-center justify-between pt-1 border-t border-slate-50">
                    <span class="text-blue-600 font-extrabold text-sm">Rp <?= number_format($row_serupa['harga'], 0, ',', '.'); ?></span>
                  </div>
                </a>
        <?php 
            }
